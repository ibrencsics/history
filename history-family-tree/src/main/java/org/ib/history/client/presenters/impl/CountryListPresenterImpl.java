...
public class CountryListPresenterImpl extends AsyncDataProvider<CountryData> implements CountryListPresenter {
...
//        countryListView.refreshGrid();
    ...
    protected void onRangeChanged(HasData<CountryData> countryDataHasData) {
        backendService.getCountries("DE", new AsyncCallback<List<CountryData>>() {
            @Override
            public void onFailure(Throwable throwable) {
                Window.alert("Error " + throwable.getMessage());
            }
            ...
            public void onSuccess(List<CountryData> countryDataList) {
                updateRowData(0, countryDataList);
            }
        });
    }
}
